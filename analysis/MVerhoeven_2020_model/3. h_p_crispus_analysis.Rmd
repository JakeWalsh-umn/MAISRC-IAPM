...
#' Are our datasets complete for the parameters we want to estimate on?
 ...
 #de-duplicate
 pcri.e <- pcri.e %>% 
  distinct(Lake.ID, Year, APRMAYPcri, .keep_all = TRUE)
    
  pcri.p <- pcri.p %>% 
  distinct(Lake.ID, Year, JUNPcri, .keep_all = TRUE)
...
```{r mixed effects models}
  ...
  #' ### Early Survey Freq
    glme.foc.e <- glmer(foc_clp ~ lag1 + cytrt + PrevAUGSecchi.sc + Snow.sc + Ice_off.sc + (1 | Lake.ID) + (1 | obs), family = binomial(logit), data = pcri.e, weights = nsamp)
    summary(glme.foc.e)
  ...
  #try 2, no Ice off--------------
        #' ### Early Survey Freq
    glme.foc.e.2 <- glmer(foc_clp ~ lag1 + cytrt + PrevAUGSecchi.sc + Snow.sc + (1 | Lake.ID) + (1 | obs), family = binomial(logit), data = pcri.e, weights = nsamp)
    summary(glme.foc.e.2)
    
    
      #' ### Late Survey Freq
    glme.foc.p.2 <- glmer(foc_clp ~ Trt + cytrt + PrevAUGSecchi.sc + Snow.sc +
    (1 | Lake.ID) + (1 | obs), family = binomial(logit), data = pcri.p, , weights = nsamp)
...
```{r Mike additional modeling}
  ...
  #' ### Late Surveys Freq
    ...
    #peak foc
    glme.foc.p.4 <- glmer(foc_clp~ Trt + cytrt  +
                          (1 | Lake.ID)+ (1 | obs), family = binomial(logit), data = pcri.p, weights = nsamp)
    summary(glme.foc.p.4)
    
    
    glme.foc.e.4 <- glmer(foc_clp~ lag1 + cytrt  +
